...
while i < len(orderedNFRList):
    ...
    PointsList = PointsInCircum(no_of_points, no_of_points)
    #print(PointsList)
    ik = 0
    for j in PointsList:
        x = j[0]
...
cursor = conn.cursor()
#cursor.execute('select op1, op2, pathid from TblFinalPath')
cursor.execute('select op1, op2 from TblIndividualPath')
orderedpairResult = cursor.fetchall()
colindex = 0
previd = 0
colorid = ''
